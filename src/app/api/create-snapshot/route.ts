...
export async function POST(request: NextRequest) {
  try {
    ...
    // Liquor (inventory_items) — snapshot_items FK requires inventory_items IDs
    ...
    if (liquorItems) {
      for (const item of liquorItems) {
        const full = item.current_count || 0
        ...
        snapshotItems.push({
          ...
          total_count: Math.round((full + partial) * 10) / 10,
        })
      }
    }

    // Other categories — pull for email but don't insert into snapshot_items (FK constraint)
    ...
    const totalItemCount = snapshotItems.length + 
      (beerItems?.length || 0) + (wineItems?.length || 0) + 
      (pepsiItems?.length || 0) + (glasswareItems?.length || 0)
      ...
      const emailHtml = `
        ...
        <body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; max-width: 700px; margin: 0 auto; padding: 20px;">
          ...
          <div style="background: #f8fafc; padding: 15px; border: 1px solid #e2e8f0; border-top: none;">
            <strong>${totalItemCount} items captured</strong> across all categories
          </div>
          ${sections.join('')}
