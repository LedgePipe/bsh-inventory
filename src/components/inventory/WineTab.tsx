                    ...
                    <tr key={item.id} className={`hover:bg-purple-50 transition-colors ${editedCount ? 'bg-yellow-50' : ''}`}>
                      ...
                      <td className="px-4 py-3 text-center">
                        {userRole !== 'staff' && editingPar === item.id ? (
                          ...
                          <span className={`font-medium ${userRole !== 'staff' ? 'cursor-pointer hover:text-purple-600 underline decoration-dotted underline-offset-4' : ''}`} onClick={() => userRole !== 'staff' && setEditingPar(item.id)}>{item.par_level}</span>
                        )}
                      </td>
                      <td className="px-4 py-3 text-center">
